...
def forecast_series(ts: pd.DataFrame, col: str = "Заявок", periods: int = 3) -> pd.DataFrame:
    ...
    if n >= 24 and _HW_AVAILABLE:
        # ✅ ТҮЗЕТІЛДІ: 2+ жыл деректе маусымдылық қосылды
        try:
    ...
    if fc_arr is None and n >= 4 and _HW_AVAILABLE:
        # ✅ ТҮЗЕТІЛДІ: 4-23 ай — тек тренд
        try:
            model = ExponentialSmoothing(
                values,
                trend="add",
    ...
    if fc_arr is None:
        # ✅ ТҮЗЕТІЛДІ: <4 ай — moving average fallback
