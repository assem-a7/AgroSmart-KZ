...
class ScoringEngine:
    ...
    def detect_anomalies(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        zero_share = float(((df["Причитающая сумма"] == 0) & (df["Норматив"] > 0)).mean())
        night_share = float(df["Ночная_подача"].mean())
        amount = pd.to_numeric(df["Причитающая сумма"], errors="coerce").fillna(0)
        q1 = amount.quantile(0.25)
        q3 = amount.quantile(0.75)
        iqr = q3 - q1
        if iqr > 0:
            outlier_mask = (amount < (q1 - 3 * iqr)) | (amount > (q3 + 3 * iqr))
            sigma = float(amount.std(ddof=0) or 0.0)
            if sigma > 0:
                outlier_mask = outlier_mask | ((amount - amount.mean()).abs() > 3 * sigma)
            tail_share = float(outlier_mask.mean())
        else:
            sigma = float(amount.std(ddof=0) or 0.0)
            tail_share = float((amount - amount.mean()).abs().gt(3 * sigma).mean()) if sigma > 0 else 0.0

        contamination = zero_share * 0.4 + night_share * 0.3 + tail_share * 0.3
        contamination = float(np.clip(contamination, 0.01, 0.10))
        print(
            f"ℹ️ Contamination авто: {contamination:.3f}\n"
            f"   (нөл-сомма={zero_share:.3f}, түн={night_share:.3f}, шеттік={tail_share:.3f})"
        )

        self.iso_forest = IsolationForest(
            n_estimators=300,
            contamination=contamination,   # ✅ ТҮЗЕТІЛДІ: contamination деректен авто есептеледі
            max_samples="auto",
            random_state=42,
            n_jobs=-1,
        )
    ...
    def train_xgboost(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.xgb_model = xgb.XGBClassifier(
            ...
            scale_pos_weight=scale_pos_weight,  # ✅ ТҮЗЕТІЛДІ: класс теңгерімсіздігі ескерілді
            eval_metric="logloss",
            random_state=42,
            n_jobs=-1,
            verbosity=0,
    ...
    def load(self, path: str = "models/") -> bool:
        try:
            self.iso_forest = joblib.load(os.path.join(path, "iso_forest.pkl"))
